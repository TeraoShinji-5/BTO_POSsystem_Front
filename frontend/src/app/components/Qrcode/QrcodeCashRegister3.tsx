...
export default function QrcodeReaderComponent() {
    ...
    const [quantity, setQuantity] = useState(0); // newProduct.quantityを管理するためのローカルステートを追加
    ...
    async function fetchProduct(scannedResult) {
        ...
        const res = await fetch(`tech0-gen-5-step4-studentwebapp-7.azurewebsites.net/qrcode?qrcode=${encodedQrcode}`, { cache: "no-cache" });
        if (!res.ok) {
    ...
    const handleQuantityChange = (event: any) => {
        // 入力値を数値に変換してからステートを更新
        const value = parseInt(event.target.value, 10);
        setQuantity(isNaN(value) ? 0 : value); // 数値が不正な場合は0をセット
    ...
    // 数量の直接編集を許可する場合、この関数は不要になる可能性があります
    // もし使用する場合、newProduct.quantityの値が数値であることを確認してください
    ...
    useEffect(() => {
        if (newProduct && typeof newProduct.quantity === 'number') {
            setQuantity(newProduct.quantity);
        }
    }, [newProduct]);
    ...
    const fetchAndSetTrade = async (buyTime: Date) => {
        ...
        const response = await fetch('tech0-gen-5-step4-studentwebapp-7.azurewebsites.net/trade', {
            method: 'POST',
            headers: {
    ...
    const fetchAndDealDetail = async (buyTime: Date) => {
        ...
        const response = await fetch('tech0-gen-5-step4-studentwebapp-7.azurewebsites.net/deal_detail', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
